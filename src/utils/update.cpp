...
bool check_for_updates()
{
    ...
    if (curl)
    {
        ...
        if (res == CURLE_OK)
        {
            ...
            print_debug((std::string("") + get_online_version_number() + " : " + settings_get_value(setting_local_version) + "\n").c_str());
            if (is_number(get_online_version_number()) && is_number(settings_get_value(setting_local_version)))
            {
                print_debug("nums\n");
                if (std::stod(get_online_version_number()) > std::stod(settings_get_value(setting_local_version)))
                {
                    print_debug("need up\n");
